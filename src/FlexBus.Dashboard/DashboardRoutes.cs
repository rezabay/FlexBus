    ...
    public class DashboardRoutes
    {
        ...
        public static RouteCollection GetDashboardRoutes(ISerializer serializer)
        {
            ...
            Routes.AddPublishBatchCommand(
                "/published/requeue",
                async (client, messageId) =>
                {
                    var msg = await client.Storage.GetMonitoringApi().GetPublishedMessageAsync(messageId);
                    msg.Origin = serializer.Deserialize(msg.Content);
                    await client.RequestServices.GetService<IDispatcher>().EnqueueToPublish(msg);
                });
            Routes.AddPublishBatchCommand(
                "/received/requeue",
                async (client, messageId) =>
                {
                    var msg = await client.Storage.GetMonitoringApi().GetReceivedMessageAsync(messageId);
                    msg.Origin = serializer.Deserialize(msg.Content);
                    await client.RequestServices.GetService<IDispatcher>().EnqueueToPublish(msg);
                }); 
